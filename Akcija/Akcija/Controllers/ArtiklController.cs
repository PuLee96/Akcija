...
namespace Akcija.Controllers
{
    public class ArtiklController : Controller
    {
        ...
        {
            List<Artikl> popis = baza.Artikli.ToList();
        ...
        public ActionResult DodavanjeArtikla(int? id)
        {
            ...
            artikli.Add(new Artikl { naziv_artikl = "Nedefinirano" });
        ...
        [HttpPost]
        public ActionResult DodavanjeArtikla(Artikl a)
        {
        ...
        public ActionResult UrediArtikl(int id)
        {
            ...
            Artikl a = new Artikl();
            foreach (Artikl art in baza.Artikli)
            {
                if (art.id_artikl == id)
                {
        ...
        [HttpPost]
        public ActionResult UrediArtikl([Bind(Include = "id_artikl, naziv_artikl,cijena_artikl")] Artikl art)
        {
            if (!ModelState.IsValid)
            {
        ...
        {
            Artikl art = baza.Artikli.Find(id);
            if (Request.IsAjaxRequest())
            {
                ViewBag.IsUpdate = false;
